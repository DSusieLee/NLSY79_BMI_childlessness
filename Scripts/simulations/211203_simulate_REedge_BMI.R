...
saveRDS(sim_disabled_f, './Results/211204_sim_REdisabeled_f.rds')
saveRDS(sim_disabled_m, './Results/211204_sim_REdisabeled_m.rds')
saveRDS(sim_enabled_f, './Results/211204_sim_REenabeled_f.rds')
saveRDS(sim_enabled_m, './Results/211204_sim_REenabeled_m.rds')

# visualize ---------------------------------------------------------------

sim_enabled_f <- readRDS('./Results/211204_sim_REenabeled_f.rds')
sim_disabled_f <- readRDS('./Results/211204_sim_REdisabeled_f.rds')
sim_enabled_m <- readRDS('./Results/211204_sim_REenabeled_m.rds')
sim_disabled_m <- readRDS('./Results/211204_sim_REdisabeled_m.rds')


#women
df <- rbind(sim_enabled_f %>% mutate(re = TRUE), 
           sim_disabled_f %>% mutate(re = FALSE)) 
df$actual = rep(c(0.038, -0.022, -0.06), nrow(df)/3)
...
ggplot(df, aes(x = childless, fill = re)) +
  ...
  labs(fill = "Scenario",
       title = "Early childbearing differs by race/ethnicity (dark bars) or not (light bars)") +
  xlab("Difference in P(childless)") +
  facet_wrap(~ pairwise)

ggsave('./Figures/211204_simulate_RE_f.png', type = "cairo")

#men
dm <- rbind(sim_enabled_m %>% mutate(re = TRUE), 
            sim_disabled_m %>% mutate(re = FALSE)) 
dm$actual = rep(c(0.038, -0.022, -0.06), nrow(dm)/3)
...
ggplot(dm, aes(x = childless, fill = re)) +
  ...
  theme_ipsum(axis_title_size = 15,
              ...
              plot_margin = margin(10,10,10,10)) +
  labs(fill = "Scenario",
       title = "Race - Early Parenthood absent (green) vs. present (purple)") +
  xlab("Difference in P(childless)") +
  facet_wrap(~ pairwise)

ggsave('./Figures/211204_simulate_RE_m.png', type = "cairo")
